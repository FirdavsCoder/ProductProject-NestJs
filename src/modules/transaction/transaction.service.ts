...
import { CountException, TransactionNotFoundException } from './exception/transaction.exception';
...
export class TransactionService implements ITransactionService {
  ...
  async create(transaction: CreateTransactionDto, currentUser: UserEntity): Promise<ResData<TransactionEntity>> {
    await this.userservice.findOne(transaction.userId);
    const {data: foundProduct} = await this.producService.findOne(transaction.productId);
    if (
  ...
  async findAll():Promise<ResData<TransactionEntity[]>>  {
    ...
    return new ResData<TransactionEntity[]>("All Transactions", 200, foundTransactions);
  }
  ...
  async findOneByUserId(id: number):Promise<ResData<TransactionEntity[]>> {
    ...
    return new ResData<TransactionEntity[]>("Transaction by userID", 200, foundTransaction);
  }
  ...
  async findOneByProductId(id: number): Promise<ResData<TransactionEntity[]>> {
    ...
    return new ResData<TransactionEntity[]>("Transactions by productID", 200, foundTransaction);
  }
  ...
  async findOneById(id: number): Promise<ResData<TransactionEntity>> {
    ...
    return new ResData<TransactionEntity>("Transactions found by id", 200, foundTransaction);
  }
